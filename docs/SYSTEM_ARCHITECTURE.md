...
The application uses a local-first single-page architecture. Document bytes are loaded into browser memory, preview rendering is handled through `pdf.js`, PDF composition is handled through `pdf-lib`, and browser-side helper modules isolate utility, overlay, split, export, and stamp-editor concerns from the main runtime.
